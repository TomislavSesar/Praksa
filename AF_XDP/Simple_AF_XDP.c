...
#define MAX_IFACES 1
#define ETH_LENGHT sizeof(struct ether_header)
...
xdp_flags -> holds parametrs that control how XDP program is going to attach to the interface
xsk_bind_flags -> control how XDP socket is going to bind to an interface
umem_flags -> flags that configure UMEM
umem_frames_nr -> number of frames in UMEM
umem_fill_size -> number of slots in fill_ring where UMEM descriptors can be put in
umem_comp_size ->   number of slots in completion_ring where UMEM descriptors can be put in
packet_handler_tid -> thread that reads our packets that have arrived to our UMEM or 
                    sending them 
packet_handler_thread_started -> bool flag that tells us if our thread is activated successfully
*/
...
static struct argp_option options[] = {
    {"interface", 'i', "string", 0, "This is argument uses the inteface that is going to be used"},
    {"copy",      'C', 0,        0, "Force XDP copy mode"},
    {0}
};
...
static struct xsk_umem_info *xsk_configure_umem(struct xdp_eth_context *ctx) {
    ...
    (in our case by default it's zero)
    */
    ...
    /*
    xsk_umem__create -> creates our UMEM
    */

    int create = xsk_umem__create(&ctx->umem->umem, ctx->buffer, ctx->umem_size,
                                  &ctx->umem->fq, &ctx->umem->cq, &umem_cfg);

    //Error handeling if our UMEM is not created 
    if (create) {
        free(ctx->umem);
        ctx->umem = NULL;
        return NULL;
...
static void xsk_populate_fill_ring(struct xdp_eth_context *ctx) {
    ...
    /*
    xsk_ring_prod__reserve -> reserves free slots in fill ring 
    */
    ...
    /*
    xsk_ring_prod__fill_addr -> saves descriptors that points to free memory chunks in UMEM
    */
    ...
    /*
    xsk_ring_prod__submit -> tells kernal that umem_fill_size is ready to use
    */
    xsk_ring_prod__submit(&umem->fq, ctx->umem_fill_size);
}

static struct xsk_socket_info *
xsk_configure_socket(struct xdp_eth_context *ctx) {
    ...
    /*
    xsk_socket__create -> creates XDP socket
    */
...
static void print_packet(uint8_t *pkt, uint32_t len) {
    ...
    strncpy(src_mac, ether_ntoa((const struct ether_addr *)eth->ether_shost), 18);
    strncpy(dst_mac, ether_ntoa((const struct ether_addr *)eth->ether_dhost), 18);
    src_mac[17] = '\0';
    dst_mac[17] = '\0';

    const struct iphdr *ip = (const struct iphdr *)(pkt + ETH_LENGHT);
    ...
    if (ip->protocol == IPPROTO_TCP) {
        const struct tcphdr *tcph =
            (const struct tcphdr *)(pkt + ETH_LENGHT + ip->ihl * 4);
        ...
        const struct udphdr *udph =
            (const struct udphdr *)(pkt + ETH_LENGHT + ip->ihl * 4);
...
static int handle_receive_packets(struct xsk_socket_info *xsk, struct xdp_eth_context *ctx) {

    unsigned int rcvd, i;
    uint32_t idx_rx = 0, idx_fq = 0;
    int ret;

    //xsk_ring_cons__peek -> checks if there are new packets in RX ring without syscall
    ...
    //xsk_ring_prod__reserve -> reserves free space in producer ring (in this case fill ring)
    ...
    for (i = 0; i < rcvd; i++) {
        ...
        //xsk_ring_cons__rx_desc -> returns descriptor in rx ring
        const struct xdp_desc *desc = xsk_ring_cons__rx_desc(&xsk->rx, idx_rx++);
        print_packet((uint8_t *)xsk_umem__get_data(xsk->umem->buffer, desc->addr), desc->len);
        *xsk_ring_prod__fill_addr(&xsk->umem->fq, idx_fq++) = desc->addr;
    }

    //xsk_ring_prod__submit -> moves pointer in fill ring for rcvd spaces 
    xsk_ring_prod__submit(&xsk->umem->fq, rcvd);

    //xsk_ring_cons__release -> moves consumer pointer in RX ring for rcvd spaces 
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        printf("Deinitializing and init again after timeout\n");
