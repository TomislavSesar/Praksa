...
void obrada_paketa(u_char *args, const struct pcap_pkthdr *header,
                   const u_char *packet)
{
    char *protocol = "#";
    char *src_ip;
    ...
    // Packet's first header is eth header that conatins source and destination
    // mac address
    //  ether_header strcuture contains: uint8_t ether_dhost(dst_mac), uint8_t
    //  ether_shost(src_mac), uint16_t ether type
    const struct ether_header *eth = (const struct ether_header *)packet;

    // ehther_shost, ether_dhost need to be cast because function ether_nota
    // only accepts ether_addr structure, ether_nota turns raw bytes into
    ...
    */
    ...
    // Lenght of ETH header is fixed = 14, bud leght of IP can vary from 20 to
    // 60 bytes, for that reason we need to use ip->ihl which tell us ip header
    // lenght and multiply it with 4 to get true size in bytes
...
int main(int argc, char *argv[])
{
    ...
    struct pcap_pkthdr header; // metadata about our packet (time, number of
                               // capture bytes, real packet size)

    const u_char *packet; // packet, it is u_char type because our packet is
    ...
    // 1 = promiscuosu mode, 1000 = timeout how long does it wait for packet,
    // errbuf = string taht stores error

    handle = pcap_open_live(device, BUFSIZ, 1, 1000, errbuf); // setting session
                                                              // id
    ...
    pcap_loop(handle, -1, obrada_paketa,
              (u_char *)&counter); // handle = id session, -1 = number of packet
                                   // to sniff before stopping(-1 means infinte)
